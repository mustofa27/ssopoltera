...
@section('content')
    <div class="card">
        <h2 class="heading-top-reset">Edit User</h2>
        ...
        <form method="POST" action="{{ route('users.update', $user) }}">
            ...
            @php
                ...
                $selectedSupportUnitIds = old('support_unit_ids', $user->affiliations->where('is_primary', false)->pluck('support_unit_id')->filter()->map(fn ($id) => (int) $id)->all());
            @endphp
            ...
            <div class="grid mb-16">
                ...
                <div>
                    ...
                    <input class="input" id="job_title" type="text" name="job_title" value="{{ old('job_title', $user->job_title) }}">
                </div>
                <div>
                ...
                <div>
                    ...
                    <select class="input" id="employee_type" name="employee_type">
                        ...
                        @foreach($employeeTypes as $value => $label)
                            <option value="{{ $value }}" {{ old('employee_type', $user->employee_type) === $value ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label class="label" for="nip">NIP</label>
                    <input class="input" id="nip" type="text" name="nip" value="{{ old('nip', $user->nip) }}">
                </div>
                <div>
                    <label class="label" for="nrp">NRP</label>
                    <input class="input" id="nrp" type="text" name="nrp" value="{{ old('nrp', $user->nrp) }}">
                </div>
                <div>
                    ...
                    <select class="input" id="primary_department_id" name="primary_department_id">
                        ...
                        @foreach($departments as $department)
                            <option value="{{ $department->id }}" {{ (string) old('primary_department_id', $primaryAffiliation?->department_id ?? $primaryAffiliation?->programStudy?->department_id) === (string) $department->id ? 'selected' : '' }}>{{ $department->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <select class="input" id="primary_program_study_id" name="primary_program_study_id">
                        ...
                        @foreach($programStudies as $programStudy)
                            <option value="{{ $programStudy->id }}" {{ (string) old('primary_program_study_id', $primaryAffiliation?->program_study_id) === (string) $programStudy->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select class="input" id="primary_support_unit_id" name="primary_support_unit_id">
                        ...
                        @foreach($supportUnits as $supportUnit)
                            <option value="{{ $supportUnit->id }}" {{ (string) old('primary_support_unit_id', $primaryAffiliation?->support_unit_id) === (string) $supportUnit->id ? 'selected' : '' }}>{{ $supportUnit->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="choice-group">
                    @foreach($supportUnits as $supportUnit)
                        <label class="choice-item">
                            <input type="checkbox" name="support_unit_ids[]" value="{{ $supportUnit->id }}" {{ in_array($supportUnit->id, $selectedSupportUnitIds) ? 'checked' : '' }}>
